...
BOOL Memory::Setup() {
	DeviceHandler = CreateFileA(("\\\\.\\DeviceHandler"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
...
uint64_t Memory::GetModuleBase(const std::string& module_name) {
	...
	if (DeviceIoControl(DeviceHandler, ioctl_get_module_base, &request, sizeof(K_GET_BASE_MODULE_REQUEST), &request, sizeof(K_GET_BASE_MODULE_REQUEST), &junk, 0)) {
...
bool Memory::ReadMemoryFromKernel(int processId, uint64_t address, void* buffer, size_t size)
{
	...
	DWORD junk = 0;
	auto status = DeviceIoControl(DeviceHandler, ioctl_read_memory, &request, sizeof(K_RW_REQUEST), &request, sizeof(K_RW_REQUEST), &junk, (LPOVERLAPPED)NULL);
...
bool Memory::WriteMemoryFromKernel(int processId, uint64_t address, void* buffer, size_t size)
{
	...
	DWORD junk = 0;
	auto status = DeviceIoControl(DeviceHandler, ioctl_write_memory, &request, sizeof(K_RW_REQUEST), &request, sizeof(K_RW_REQUEST), &junk, (LPOVERLAPPED)NULL);
	if (!NT_SUCCESS(status)) {
		return false;
	}
